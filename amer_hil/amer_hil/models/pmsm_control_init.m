...
% Controller timing.
PlantVariant = 13;          % Keeps the legacy top-level plant variant resolvable.
L = Ld;                     % Legacy RLUb plant blocks expect scalar L.
Ub = Udc/2;                 % Legacy half-bridge plant bias voltage.
Uout0 = Ub;                 % Legacy PWM initial output voltage.
D0 = Uout0/Udc;             % Legacy PWM initial duty ratio.
U_FS = Udc/0.8;             % Legacy fixed-point voltage full scale.
I_FS = IabcFS/0.8;          % Legacy fixed-point current full scale.
...
SampleMode = 1;             % 1: sample at both triangular carrier peaks.

if SampleMode == 1
    Ts = 1/fsw/2;           % Current controller sample time.
else
    Ts = 1/fsw;
end
...
phim = pi/3;                % Desired phase margin.
phi0 = pi/2 - phim;         % Remaining phase for controller design.
wc = (2/3)*phi0/Td;         % Current-loop crossover frequency.
TI = 1/(wc*tan(phi0/3));    % Current-loop integral time constant.
...
fcpu = 80e6;                % C2000 CPU clock used by the original skeleton.
Deadtime = 0e-6;            % Deadtime in seconds.
TBPER = fcpu/fsw/2;
TBPER2 = fcpu/(2*fsw)-1;
RED = Deadtime*fcpu;
FED = Deadtime*fcpu;
